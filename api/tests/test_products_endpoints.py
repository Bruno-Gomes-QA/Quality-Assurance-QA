...
def test_given_post_products_when_get_reponse_then_added_product(client):
    data = {'product_name': 'Test', 'product_description': 'Test', 'buy_price': 10.0, 'sale_price': 20.0, 'stock': 10.0, 'department_id': 1}
    now = datetime.now().strftime('%a, %d %b %Y %H:%M:%S GMT')
    expect_data = {'id': 1, 'product_name': 'Test', 'product_description': 'Test', 'buy_price': 10.0, 'sale_price': 20.0, 'stock': 10.0, 'department_id': 1, 'created_at': now, 'updated_at': now}
